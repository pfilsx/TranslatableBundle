 ...
 * @package Pfilsx\Translatable\Maker
 * @codeCoverageIgnore
 */
class MakeTranslatable extends AbstractMaker implements InputAwareMakerInterface
{
    private $fileManager;
    private $doctrineHelper;
    private $generator;

    public function __construct(FileManager $fileManager, DoctrineHelper $doctrineHelper, Generator $generator)
    {
    ...
    public function configureCommand(Command $command, InputConfiguration $inputConf)
    {
        $command
            ...
            ->addArgument('name', InputArgument::OPTIONAL, sprintf('Class name of the translatable entity to create or update (e.g. <fg=yellow>%s</>)', Str::asClassName(Str::getRandomTerm())))
    ...
    public function generate(InputInterface $input, ConsoleStyle $io, Generator $generator)
    {
        ...
        $translationClassDetails = $generator->createClassNameDetails(
            $input->getArgument('name') . 'Translation',
            'Entity\\Translation\\'
        );
        $classExists = class_exists($entityClassDetails->getFullName());
        $translationExists = class_exists($translationClassDetails->getFullName());
        if (!$classExists) {
            $entityClassGenerator = new TranslatableEntityClassGenerator($generator);
        ...
        if (!$translationExists) {
            $translationClassGenerator = new TranslatableEntityClassGenerator($generator);
            $translationPath = $translationClassGenerator->generateTranslationClass(
                $translationClassDetails,
                $input->getOption('api-resource')
            );
            $generator->writeChanges();
        }
        if (!$this->doesEntityUseAnnotationMapping($entityClassDetails->getFullName())) {
            throw new RuntimeCommandException(sprintf('Only annotation mapping is supported by make:translatable, but the <info>%s</info> class uses a different format.', $entityClassDetails->getFullName()));
        }
        if (!$this->doesEntityIsTranslatable($entityClassDetails->getFullName())) {
            throw new RuntimeCommandException(sprintf('Only translatable entities are supported by make:translatable, but the <info>%s</info> class does not implement TranslatableInterface.', $entityClassDetails->getFullName()));
        }
        if (!$this->doesTranslationIsTranslation($translationClassDetails->getFullName())) {
            throw new RuntimeCommandException(sprintf('Translation class must implement TranslationInterface, but the <info>%s</info> class does not implement TranslationInterface.', $translationClassDetails->getFullName()));
        }
        if ($classExists && $translationExists) {
            $entityPath = $this->getPathOfClass($entityClassDetails->getFullName());
            $translationPath = $this->getPathOfClass($translationClassDetails->getFullName());
            $text = [
                'Your entities already exist! So let\'s add some new fields!',
            ];
        } elseif ($classExists && !$translationExists) {
            $entityPath = $this->getPathOfClass($entityClassDetails->getFullName());
            $text = [
                'Your translatable entity already exists and translation entity was generated! So let\'s add some new fields!'
            ];
        } elseif ($translationExists && !$classExists) {
            $translationPath = $this->getPathOfClass($translationClassDetails->getFullName());
            $text = [
                'Your translation entity already exists and translatable entity was generated! So let\'s add some new fields!'
            ];
        } else {
            $text = [
                '',
                'Entities generated! Now let\'s add some fields!',
        ...
        $translationManipulator = $this->createClassManipulator($translationPath, $io, $overwrite);
        $isFirstField = true;
        while (true) {
            ...
            $fileManagerOperations = [
                ...
                $translationPath => $translationManipulator
            ];
            if (is_array($newField)) {
                $annotationOptions = $newField;
                unset($annotationOptions['fieldName']);
                unset($annotationOptions['translatable']);
                if ($newField['translatable']){
                    $translationManipulator->addEntityField($newField['fieldName'], $annotationOptions);
                } else {
    ...
    private function askForNextField(ConsoleStyle $io, array $fields, string $entityClass, bool $isFirstField)
    {
        ...
        $data['translatable'] = (bool)$io->confirm('Is this field translatable', false);
        return $data;
    }
    ...
    private function doesEntityIsTranslatable(string $className)
    {
        return is_subclass_of($className, TranslatableInterface::class);
    }

    private function doesTranslationIsTranslation(string $className)
    {
